...
export const useMusicStore = defineStore("music", () => {
  ...
  const defaultLibrary: Track[] = [
    {
      ...
      icon: "potted_plant",
      description:
        "The iconic focus soundtrack. Steady, gentle beats for deep concentration.",
    },
    {
      id: "Qo4JIT8jMtI",
      name: "Mountain Rain",
    ...
    },
    {
      ...
      icon: "rocket_launch",
      description:
        "Cosmic journey in deep space 🔭White noise will help you to relax 💤",
    },
    {
      id: "zr5JjgOMXeQ",
      name: "Quiet Cafe",
  ...
  const personalTracks = useLocalStorage<Track[]>(
    "pomohaven_personal_tracks_v1",
    [],
  );
  ...
  */
  ...
  const addTrack = async (id: string, name: string, genre: string) => {
    // Avoid duplicates
    if (
      personalTracks.value.some((t) => t.id === id) ||
      defaultLibrary.some((t) => t.id === id)
    ) {
      return;
    }
    ...
    if (authStore.user) {
      ...
      if (error) {
        ...
        console.error(
          "[MusicStore] Failed to save track to DB:",
          error.message,
          error,
        );
  ...
  const getFeaturedTracks = () => {
    ...
    const featuredCurated = [
      "X4VbdwhkE10",
      "Qo4JIT8jMtI",
      "kXUnJ61KxRE",
      "zr5JjgOMXeQ",
    ];
    const curated = defaultLibrary.filter((t) =>
      featuredCurated.includes(t.id),
    );
